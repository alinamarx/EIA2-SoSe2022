namespace Canvas {
  ...
  function handleLoad(_event: Event): void {
    ...
    let colorArray: string[] = ["#4661EE", "#EC5657", "#1BCDD1", "#8FAABB", "#B08BEB", "#3EA0DD", "#F5A52A", "#23BFAA", "#FAA586", "#EB8CC6"];
    ...
    for (let i: number = Math.floor(Math.random() * ( 30 + 1)); i < canvas.width; i += 30) {
      crc2.beginPath();
      crc2.arc(i, Math.floor(Math.random() * (canvas.height + 1)), Math.floor(Math.random() * (100 + 1)), 0, 2 * Math.PI);
      crc2.closePath();
      crc2.stroke();

      for (let i: number = Math.floor(Math.random() * ( 100 + 1)); i < canvas.height; i += 100) {
        crc2.beginPath();
    ...
    for (let i:  number = 0; i <= 10; i++) {
      ...
      let randomNumberStar: number = Math.floor(Math.random() * ( 30 + 1));
      for (let j: number = 8; j <= randomNumberStar; j++) {
        crc2.beginPath();
        crc2.lineTo(100, 100);
        crc2.lineTo(10, 10);
        crc2.closePath();
